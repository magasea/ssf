...
public class BroadcastMessageConsumers {
  ...
  @RabbitListener(bindings = @QueueBinding(
      value = @Queue(value = RabbitMQConstants.QUEUE_USERINFO_BASE
          + RabbitMQConstants.OPERATION_TYPE_UPDATE_UIPROD,
          durable = "false"),
  ...
  public void receiveTradeMessage(TrdPayFlow trdPayFlow, Channel channel, @Header(AmqpHeaders
    ...
    try {
      ...
      if (trdPayFlow.getTrdStatus() == TrdOrderStatusEnum.WAITPAY.getStatus() ||
        ...
        //等待支付金额就是下单请求时候的金额
        mongoUiTrdLog
            .setAmount(TradeUtil.getBigDecimalNumWithDiv100(trdPayFlow.getTradeTargetSum()));
        ...
        //等待赎回份额就是下单请求时候的份额
        mongoUiTrdLog
            .setAmount(TradeUtil.getBigDecimalNumWithDiv100(trdPayFlow.getTradeTargetShare()));
...
//            }
  ...
  public void receiveConfirmInfoUpdateProdQty(MongoUiTrdZZInfo mongoUiTrdZZInfo, Channel channel,
      @Header
          (AmqpHeaders.DELIVERY_TAG) long tag) throws Exception {
    try {
      logger.info("received mongoUiTrdZZInfo to update assects with userProdId:{} "
...
//            uiProductDetailRepo.save(productDetail);
  ...
  private Long getFundUnitNet(String fundCode, List<FundNetInfo> fundNetInfos, String tradeDate) {
    for (FundNetInfo fundNetInfo : fundNetInfos) {
      if (fundNetInfo.getFundCode().equals(fundCode) && fundNetInfo.getTradedate()
          .equals(tradeDate)) {
...
//    }
  ...
  public void receiveBuyOrderMsg(PayOrderDto payOrderDto, Channel channel, @Header
    ...
    logger.info("received message in :{}",
        RabbitMQConstants.OPERATION_TYPE_UPDATE_BUY_PENDINGRECORDS);
    ...
    for (TrdOrderDetail trdOrderDetail : trdOrderDetails) {
      ...
      if (CollectionUtils.isEmpty(mongoPendingRecords)) {
        ...
        logger.info("it is normal that in current stage there should be no pendingRecord "
                ...
                .getUserProdId(), trdOrderDetail.getFundCode(),
            (trdOrderDetail.getOrderId() + trdOrderDetail.getId()));
        Query queryWithEmptyOrderId = new Query();
        queryWithEmptyOrderId
            .addCriteria(Criteria.where("user_prod_id").is(trdOrderDetail.getUserProdId()).and
  ...
  private void updateHistorySellPendRecordByProdDtlSellDTO(
      MongoPendingRecords mongoPendingRecordsHistory,
    ...
    mongoPendingRecordsHistory
        .setTradeTargetSum(TradeUtil.getLongNumWithMul100(prodDtlSellDTO.getTargetSellAmount
            ()));
  }
  ...
  public void receiveFailedMsg(TrdPayFlow trdPayFlow, Channel channel, @Header
    ...
    Query query = new Query();
    query.addCriteria(Criteria.where("user_prod_id").is(trdPayFlow.getUserProdId()).and("outside_order_id").is
...
//    }
  ...
  private MongoPendingRecords getPatchMongoPendingRecord(TrdOrderDetail trdOrderDetail) {
    ...
    mongoPendingRecordsPatch
        .setOutsideOrderId(trdOrderDetail.getOrderId() + trdOrderDetail.getId());
  ...
  private MongoPendingRecords getPatchMongoPendingRecord(TrdPayFlow trdPayFlow) {
    ...
    if (CollectionUtils.isEmpty(trdOrderDetails)) {
      logger.error(
          "Failed to find corresponding orderDetail info with user_prod_id:{} and order_id:{} and "
              + "fund_code:{}", trdPayFlow.getUserProdId(), trdPayFlow.getOutsideOrderno(),
          trdPayFlow.getFundCode());
      return null;
    } else {
  ...
  private MongoPendingRecords getPatchMongoPendingRecordFromZZInfo(
    ...
    try {
      orderId = TradeUtil
          .getOrderIdByOutsideOrderNo(mongoUiTrdZZInfo.getOutSideOrderNo(), mongoUiTrdZZInfo
              .getOrderDetailId());
      trdOrderDetails = orderRpcService
    ...
    if (CollectionUtils.isEmpty(trdOrderDetails)) {
      logger.error(
          "Failed to find corresponding orderDetail info with user_prod_id:{} and order_id:{} and "
              + "fund_code:{}", mongoUiTrdZZInfo.getUserProdId(),
          mongoUiTrdZZInfo.getOutSideOrderNo(),
    ...
    if (MonetaryFundEnum.containsCode(mongoUiTrdZZInfo.getFundCode())) {
      ...
      Long navadj = getMoneyCodeNavAdjByDate(mongoUiTrdZZInfo.getFundCode(),
          mongoUiTrdZZInfo.getApplyDate());

      BigDecimal abstractShares = TradeUtil
          .getBigDecimalNumWithDivOfTwoLongAndRundDown(confirmedQuantity * 1000000L, navadj);
      mongoPendingRecordsPatch.setAbstractTargetShare(abstractShares.longValue());
      mongoPendingRecordsPatch.setAbstractConfirmShare(abstractShares.longValue());
    }
    ...
    mongoPendingRecordsPatch.setFundCode(mongoUiTrdZZInfo.getFundCode());
    mongoPendingRecordsPatch.setCreatedDate(TradeUtil.getUTCTime());
